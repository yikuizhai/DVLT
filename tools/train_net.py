...
class BestCheckpointer(HookBase):
  ...
  def after_step(self):
    ...
    curr_val = self.trainer.storage.latest().get('Generic-E2E_RESULTS/hmean', 0)
    # curr_val = self.trainer.storage.latest().get('Strong-E2E_RESULTS/hmean', 0)
    '''这里做了小改动'''
    import math
    ...
    try:
        _ = self.trainer.storage.history('Generic-E2E_RESULTS/hmean')
        # _ = self.trainer.storage.history('Strong-E2E_RESULTS/hmean')
    except:
        self.trainer.storage.put_scalar('MAX_Generic-E2E_RESULTS/hmean', curr_val)
        # self.trainer.storage.put_scalar('MAX_Strong-E2E_RESULTS/hmean', curr_val)

    max_val = self.trainer.storage.history('MAX_Generic-E2E_RESULTS/hmean')._data[-1][0]
    # max_val = self.trainer.storage.history('MAX_Strong-E2E_RESULTS/hmean')._data[-1][0]
