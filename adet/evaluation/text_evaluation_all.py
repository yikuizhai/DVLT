...
class TextEvaluator():
    ...
    def sort_detection(self, temp_dir):
        ...
        if "totaltext" in self.dataset_name:
            ...
            if not self.lexicon_type == None:
                lexicon_path = '/media/WD_2T/chill_research/deepsolo/datasets/ctw1500/weak_voc_new.txt'
                lexicon_fid=open(lexicon_path, 'r')
                pair_list = open('/media/WD_2T/chill_research/deepsolo/datasets/ctw1500/weak_voc_pair_list.txt', 'r')
                pairs = dict()
                lexicon_fid=open(lexicon_path, 'r')
                lexicon=[]
                for line in lexicon_fid.readlines():
        ...
        for i in files:
            if "ic15" in self.dataset_name:
                ...
                if self.lexicon_type==3:
                    ...
                    lexicon_path = '/media/WD_2T/chill_research/deepsolo/datasets/ic15/ic15/new_strong_lexicon/new_voc_img_' + str(int(i.split('/')[-1].split('.')[0])) + '.txt'
                    lexicon_fid=open(lexicon_path, 'r')
                    # pair_list = open('datasets/ic15/new_strong_lexicon/pair_voc_img_' + str(int(i.split('/')[-1].split('.')[0])) + '.txt')
                    pair_list = open('/media/WD_2T/chill_research/deepsolo/datasets/ic15/ic15/new_strong_lexicon/pair_voc_img_' + str(int(i.split('/')[-1].split('.')[0])) + '.txt')
                    pairs = dict()
                    for line in pair_list.readlines():
                        line=line.strip()
                        word = line.split(' ')[0].upper()
    ...
    def evaluate_with_official_code(self, result_path, gt_path):
        if "ic15" in self.dataset_name:
            return text_eval_script_ic15.text_eval_main_ic15(det_file="/media/WD_2T/chill_research/deepsolo/tools/" + result_path, gt_file="/media/WD_2T/chill_research/deepsolo/" + gt_path, is_word_spotting=self._word_spotting)
        else:
            ...
            return text_eval_script.text_eval_main(
                det_file="/media/WD_2T/chill_research/deepsolo/tools/" + result_path,
                gt_file="/media/WD_2T/chill_research/deepsolo/" + gt_path, is_word_spotting=self._word_spotting)
    def evaluate(self):
