...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, text_memory_resized_dec):
        src_flatten = []
        mask_flatten = []
        ...
        query_embed = text_memory_resized_dec + query_embed
        # query_embed = text_memory_resized_dec
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.com_content = nn.Linear(d_model, d_model // 2)
        self.com_pos = nn.Linear(d_model, d_model // 2)
    ...
    def forward(
        ...
        con = self.com_content(tgt)
        pos = self.com_pos(query_pos)
        q = k = torch.cat((con, pos), dim=-1)
        ...
        con_inter = self.com_content(tgt_inter)
        pos = self.com_pos(query_pos)
        q = torch.cat((con_inter, pos), dim=-1)
...
class DeformableCompositeTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.return_intermediate:
            ...
            # return torch.stack(intermediate), torch.stack(intermediate_reference_points), s, w

        return output, reference_points
        # return output, reference_points, s, w
