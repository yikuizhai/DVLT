...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if "polyline" in annos[0]:
        ...
        instance.polyline = torch.as_tensor(np.array(polys), dtype=torch.float32)

    if "boundary" in annos[0]:
        boundary = [obj.get("boundary", []) for obj in annos]
        instance.boundary = torch.as_tensor(np.array(boundary), dtype=torch.float32)

    if "text" in annos[0]:
        texts = [obj.get("text", []) for obj in annos]
        instance.texts = torch.as_tensor(np.array(texts), dtype=torch.int32)
